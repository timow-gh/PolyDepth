...
void outputPerformance(const char* filename)
{
  ...
  for (int i = 0; i < nframes - 1; i++)
  {
    ...
    fout << i << "   \t " << tframes[i] * 1000 << "   \t" << nIter[i] << "   \t" << GlobalPD[i].X() << "   \t " << GlobalPD[i].Y()
         << "   \t " << GlobalPD[i].Z() << endl;
  }
  cout << "Stop!" << endl;
}
...
void outputXformTOCs(const char* filename)
{
  ...
  for (int i = 0; i < nframes - 1; i++)
  {
    ...
    fout << i << " " << xform->R[0][0] << " " << xform->R[0][1] << " " << xform->R[0][2] << " " << xform->R[1][0] << " " << xform->R[1][1]
         << " " << xform->R[1][2] << " " << xform->R[2][0] << " " << xform->R[2][1] << " " << xform->R[2][2] << " " << xform->T[0] << " "
