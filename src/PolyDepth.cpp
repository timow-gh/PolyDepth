...
void GuessContactFreeSpace(Transform* object_1_pose,
                           C2A_Model* object_1,
                           ...
                           Transform& output_space)
{
  ...
  PQP_REAL max_bbox_size =
      sqrt(object_2->b->d[0] * object_2->b->d[0] + object_2->b->d[1] * object_2->b->d[1] + object_2->b->d[2] * object_2->b->d[2]);
  PQP_REAL max_bbox_size1 =
      sqrt(object_1->b->d[0] * object_1->b->d[0] + object_1->b->d[1] * object_1->b->d[1] + object_1->b->d[2] * object_1->b->d[2]);
  ...
  if (use_motion_coherance && clear_conf.size())
  {
    ...
    while (true)
    {
      ...
      C2A_Collide(&collide_result, obj1_rot, obj1_tran, object_1, obj2_rot, obj2_tran, object_2, C2A_FIRST_CONTACT);
  ...
  if (!found)
    printf(
        "could not select the clear points; use directional "
        "method------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
...
PolyDepthReturnValue PolyDepth(Transform* object_1_pose,
                               C2A_Model* object_1,
                               Transform* object_2_pose,
                               C2A_Model* object_2,
                               Tri* last_triangle1,
                               ...
                               PQP_REAL feature_clustering_resolution)
{
  ...
  C2A_Collide(&collide_result, temp_rot0, temp_tran0, object_1, temp_rot1, temp_tran1, object_2, C2A_FIRST_CONTACT);
  ...
  printf("time to compute colliding status %lld millisec\n", duration.count());
  ...
  duration = std::chrono::duration_cast<std::chrono::milliseconds>(current - start);
  printf("time to compute contact free space %lld\n", duration.count());
  ...
  while (true)
  {
    ...
    C2A_Solve(&begin_transform_object_1, // begin transformation of obj1
    ...
    start = std::chrono::high_resolution_clock::now();
    C2A_Distance(&distance_test_result, save_rot_matrix, save_tran_vector, object_1, temp_rot1, temp_tran1, object_2, rel_err, abs_err);
    current = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(current - start);
    printf("time to compute distance %lld\n", duration.count());
  ...
  global_penetration_depth =
      contact_configuration.Translation() - save_moving_transform.Translation(); // save_moving_transform.Translation();
  ...
  C2A_QueryContactOnly(&time_of_contact_result, R1, T1, object_1, R2, T2, object_2, absolute_contact_configuration);
  ...
  struct save_data
  {
    ...
    save_data(Coord3D& n, Coord3D& f1, Coord3D& f2)
        : feature_normal(n)
        , feature1(f1)
        , feature2(f2)
    {
  ...
  for (int jj = 0; jj < number_of_contact; jj++)
  {
    ...
    std::map<unsigned int, save_data>::iterator iter =
        feature_map.find((int)coord[2] * feature_clustering_resolution * feature_clustering_resolution +
                         (int)coord[1] * feature_clustering_resolution + (int)coord[0]);
    ...
    feature_map[(int)coord[2] * feature_clustering_resolution * feature_clustering_resolution +
                (int)coord[1] * feature_clustering_resolution + (int)coord[0]] = save_data(mnkn[jj], cpat, cpbt);
    ContactFit++;
  }
...
PolyDepthReturnValue SkinDepth(Transform* object_1_pose,
                               C2A_Model* object_1,
                               Transform* object_2_pose,
                               C2A_Model* object_2,
                               const std::vector<Coord3D>& object_1_vertices,
                               ...
                               PQP_REAL feature_clustering_resolution)
{
  ...
  C2A_Distance(&distance_test_result, temp_rot0, temp_tran0, object_1, temp_rot1, temp_tran1, object_2, rel_err, abs_err);
                                 ...
                                 int grid_resolution)
{
  ...
  for (move[0] = bmin.X(); move[0] < bmax.X(); move[0] = move[0] + unitt)
  {
    for (move[1] = bmin.Y(); move[1] < bmax.Y(); move[1] = move[1] + unitt)
    {
      for (move[2] = bmin.Z(); move[2] < bmax.Z(); move[2] = move[2] + unitt)
      {
        ...
        C2A_Distance(&dist_result, rot, move, single_triangle, rot, tran, object, rel_err, abs_err);
  ...
  // �����ִ� configuration �� ������ �񱳸� ���� ����
  // �� ���� configuration �� �Ÿ��� �� ū ���� �� configuration ������ �Ÿ������� ũ�� �Ÿ����� ���� configuration�� �����Ѵ�
  ...
  printf("number of maximally clear configuration: %zu\n", kMaxClearConf.size());

  delete single_triangle;

  object->max_clear_conf_ = kMaxClearConf;

  if (!kMaxClearConf.empty())
    return true;
  return false;
}
